...
use crate::mm::{
    translated_ref, translated_refmut, translated_str, UserBuffer, translated_byte_buffer, copy_to_user,
    get_from_user,
};
use crate::task::{
    current_process, current_task, current_user_token, exit_current_and_run_next, pid2process,
    suspend_current_and_run_next, SignalFlags, TaskStatus, wake_blocked, find_task_by_pid, Rusage, block_current_and_run_next,
};
use crate::timer::{get_time_ms, Tms, TimeSpec, add_timer, TimeVal, TimeZone};
...
pub fn sys_execve(path: *const u8, mut argv: *const *const u8, mut envp: *const *const u8,) -> isize {
    ...
    if !argv.is_null() {
        loop {
            ...
            argv_vec.push(translated_str(token, arg_str_ptr as *const u8));
            unsafe {
                argv = argv.add(1);
            }
        }
...
pub fn sys_times(tms_ptr: *mut Tms) -> isize {
    ...
    let process = task.process.upgrade().unwrap();
    let mut inner = process.inner_exclusive_access();
    let tms = translated_refmut(user_token, tms_ptr as *mut crate::timer::Tms);
...
pub fn sys_uname(utsname_ptr: *mut u8) -> isize {
    ...
    buffer.write(Some(FIELD_OFFSET * 0), b"cutecore\0");
    buffer.write(Some(FIELD_OFFSET * 1), b"xeinnious\0");
    #[cfg(target_arch = "riscv64")]
    buffer.write(Some(FIELD_OFFSET * 2), b"5.0.0-riscv64\0");
    ...
    #[cfg(target_arch = "riscv64")]
    buffer.write(Some(FIELD_OFFSET * 4), b"riscv64\0");
    #[cfg(target_arch = "loongarch64")]
    buffer.write(Some(FIELD_OFFSET * 4), b"loongarch64\0");
    buffer.write(Some(FIELD_OFFSET * 5), b"\0");
    0
    //SUCCESS
}
